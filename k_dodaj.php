...
if (isset($_POST['detal_dodaj']))
{
    ...
    {
        $question = 'INSERT INTO system_detale.detal (nazwa_detalu, maszyny_id_maszyny, stan) VALUES ("'.$_POST['nowy_detal_nazwa'].'",0,"uzycie");';
        $zapytanie = mysql_query ($question);   //dodanie detalu
        //echo $question;
        
        $question = 'SELECT id_detal, nazwa_detalu FROM system_detale.detal WHERE nazwa_detalu=\''.$_POST['nowy_detal_nazwa'].'\';';
        $res=mysql_query($question);    //pobranie id detalu
                while ($baza=mysql_fetch_array($res))
                {
                    $local_detal_id = $baza['id_detal'];
                    $nazwa_detalu_temp = $baza['nazwa_detalu'];
                } 
